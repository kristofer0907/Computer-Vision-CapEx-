...
def editor():
    return ZoneEditor(W, H, {
        "storing": rect(0.0, 0.3),
        "heating": rect(0.5, 0.8),
    })


def test_polygons_load_into_pixels(editor):
    assert editor.zones["storing"][0] == [0.0, 100.0]
    assert editor.zones["heating"][1] == [800.0, 100.0]
    assert editor.selected == "storing"          # process order, not dict order


def test_dragging_a_vertex_moves_only_that_vertex(editor):
    before = [list(v) for v in editor.zones["storing"]]
    assert editor.begin_drag(300.0, 100.0)       # the (0.3, 0.2) corner
    editor.drag_to(320.0, 90.0)
    editor.end_drag()

    assert editor.zones["storing"][1] == [320.0, 90.0]
    assert editor.zones["storing"][0] == before[0]
    assert editor.zones["storing"][2] == before[2]
    assert editor.dirty
...
def test_vertex_wins_over_interior(editor):
    ...
    editor.begin_drag(300.0, 400.0)      # the (0.3, 0.8) corner of storing
    editor.drag_to(310.0, 400.0)
    editor.end_drag()
    moved = [v for v in editor.zones["storing"] if v == [310.0, 400.0]]
    assert len(moved) == 1
    assert editor.zones["storing"][0] == [0.0, 100.0], "other corners unmoved"


def test_selected_zone_wins_a_shared_vertex():
    """Two zones meeting on an edge: you grab the one you can see selected."""
    ed = ZoneEditor(W, H, {"storing": rect(0.0, 0.5), "heating": rect(0.5, 1.0)})
    ed.select("heating")
...
def test_normalised_survives_a_resolution_change(editor):
    ...
    assert bigger.zones["storing"][0] == [0.0, 200.0]
    assert bigger.normalised()["storing"] == norm["storing"]
...
def test_save_writes_loadable_json(editor, tmp_path):
    ...
    assert set(data) == {"storing", "heating"}
...
def test_editor_handles_no_zones():
    ...
    ed.add_zone("storing")
    assert ed.selected == "storing"
    assert len(ed.zones["storing"]) == 4
