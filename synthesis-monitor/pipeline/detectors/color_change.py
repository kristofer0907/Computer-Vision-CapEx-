...
up - this detector does not have to build it:
    ...
    ctx.previous_crop(tid)
        -> that crucible's previous crop, already resized to match the current
           one so the two can be differenced directly. None on the crucible's
...
Three traps specific to this one, all mechanical rather than chemical:
  ...
  * Illumination, not chemistry. One 595 mm LED panel covers about 600 mm of
    the 780 mm platform; the far ~180 mm is a documented dim zone. A crucible that
    changed colour and a crucible that moved into the dark end look identical to a
    raw before/after diff. Either compare only within a stage where the crucible
  ...
  * A batch-wide change is normal. Every crucible darkening together during
    heating is the protocol working. If this detector fires per-crucible on an
...
class ColorChangeDetector(NotImplementedDetector):
    name = "color_change"
    description = "Gross colour shift against this crucible's own previous image"
